# PSBT Howto for Xolentum Core

Since Xolentum Core 0.17, an RPC interface exists for Partially Signed Xolentum
...
PSBT is an interchange format for Xolentum transactions that are not fully signed
...
#### Multisig with multiple Xolentum Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Xolentum Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `xolentum-cli` is used.
